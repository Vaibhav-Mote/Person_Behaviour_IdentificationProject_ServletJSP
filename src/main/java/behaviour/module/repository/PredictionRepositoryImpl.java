...
import behaviour.module.model.TextDocumentDataModel;


public class PredictionRepositoryImpl extends DBParent implements PredictionRepository{
	
	
	// fetch post id 
	public int getPostId(int postid,int predictUserID) {
		try {
			stmt=conn.prepareStatement("select pm.postid from postmaster pm inner join"
					+ " postregistrationjoin prj on prj.postid=pm.postid inner join userregistrationmaster"
					+ " rm on rm.registerid=prj.registerid where pm.postid=? and rm.registerid=?;");
			stmt.setInt(1, postid);
			stmt.setInt(2, predictUserID);
	...
	// BOW (Bag Of Words) connvert into vectors
		public int[] connvertBOWVectors(String[] str) {
	...
	// k means clustering
		public int kMeansClustering(int[] postVectors,String[] afterStopWord) {
			...
			try {
			    int[] openess_1=this.personalityconnvertsVector(afterStopWord, openessToExperience1);
			    int[] connscient_2=this.personalityconnvertsVector(afterStopWord, connscientiousness2);
			    int[] extrover_3=this.personalityconnvertsVector(afterStopWord, extroversion3);
			    int[] agreeble_4=this.personalityconnvertsVector(afterStopWord, agreeableness4);
			    int[] neurot_5=this.personalityconnvertsVector(afterStopWord, neuroticism5);
			
			    PredictionModel model = new PredictionModel();
			    model.setArr(openess_1);
			    list.add(model);
			    model = new PredictionModel();
			    model.setArr(connscient_2);
			    list.add(model);
			    model = new PredictionModel();
	...
	public int predictPersonBehavior(String post) {
		try {
			...
			// BOW (Bag Of Words) connvert into vectors
			int postVectors[]=this.connvertBOWVectors(afterStopWord);
	...
	public String[] getAllPostsCommentsLikes(int predictUserID) {
		...
		try {
			// fetch all posts
			stmt = conn.prepareStatement("select pm.postname from postmaster pm inner join postregistrationjoin "
					+ "prj on prj.postid=pm.postid inner join userregistrationmaster rm on rm.registerid=prj.registerid "
					+ "where rm.registerid=?");
			stmt.setInt(1, predictUserID);
			rs = stmt.executeQuery();
			while (rs.next()) {
			...
			stmt = conn.prepareStatement("select comment from commentmaster where registerid=?");
			stmt.setInt(1, predictUserID);
			rs = stmt.executeQuery();
			while (rs.next()) {
				TextDocumentDataModel info = new TextDocumentDataModel();
			...
			stmt = conn.prepareStatement("select pm.postname from likemaster lm inner join "
					+ "likepostjoin lpj on lpj.likeid=lm.likeid inner join postmaster pm on "
					+ "pm.postid=lpj.postid where lm.registerid=?");
	...
	// TF-IDF (Term Frequency - Inverse Document Frequency) Algorithm connvert document to vectors
	public List<PredictionModel> connvertDocumentToVectorsIFIDF(String[][] str) {
		try {
	...
	// k means clustering
				...
				try {
					...
					for (PredictionModel TFVect : TFIDFVectors) {
						double TFarr[] = TFVect.getArrayVectors();
							...
							int[] openess_1 = this.personalityconnvertsVector(afterStopWord[c], openessToExperience1);
							int[] connscient_2 = this.personalityconnvertsVector(afterStopWord[c], connscientiousness2);
							int[] extrover_3 = this.personalityconnvertsVector(afterStopWord[c], extroversion3);
							int[] agreeble_4 = this.personalityconnvertsVector(afterStopWord[c], agreeableness4);
							int[] neurot_5 = this.personalityconnvertsVector(afterStopWord[c], neuroticism5);
			...
			public List<PredictionModel> predictOverAllPersonBehavior(String[] unlabelledInformation) {
			    try {
			        ...
			        List<PredictionModel> TFIDFVectors = this.connvertDocumentToVectorsIFIDF(afterStopWord);
